...
func NewRoomService(rooms *repository.RoomRepo, users *repository.UserRepo) *RoomService {
	return &RoomService{rooms: rooms, users: users}
}

// CreateRoom creates a new room (user+)
func (s *RoomService) CreateRoom(actor *models.User, name, roomType string) (*models.Room, error) {
	switch roomType {
	case models.RoomGroup, models.RoomPublic, models.RoomPrivate:
		if name == "" {
...
func (s *RoomService) JoinRoom(actor *models.User, roomID int64) error {
	...
	if room.Type != models.RoomPublic {
